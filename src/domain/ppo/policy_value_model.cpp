...
PolicyValueModelImpl::PolicyValueModelImpl(const int64_t observation_dim, const int64_t action_dim, const int64_t hidden_dim) {
...
std::pair<torch::Tensor, torch::Tensor> PolicyValueModelImpl::policy_distribution(
    const torch::Tensor& observations
) {
    const auto latent = encode(observations);
    const auto mean = torch::tanh(actor_mean_->forward(latent));
...
torch::Tensor PolicyValueModelImpl::encode(const torch::Tensor& observations) {
